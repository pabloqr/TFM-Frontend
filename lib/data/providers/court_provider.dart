import 'package:flutter/foundation.dart';
import 'package:frontend/core/error/failure.dart';
import 'package:frontend/data/models/provider_state_enum.dart';
import 'package:frontend/domain/usecases/courts_use_cases.dart';
import 'package:frontend/features/courts/data/models/court_model.dart';
import 'package:frontend/features/courts/data/models/sport_enum.dart';

class CourtProvider extends ChangeNotifier {
  final CourtsUseCases _courtsUseCases;

  CourtProvider({required CourtsUseCases courtsUseCases}) : _courtsUseCases = courtsUseCases;

  ProviderState _state = ProviderState.initial;
  Failure? _failure;
  CourtModel _court = CourtModel(
    id: -1,
    complexId: -1,
    sport: Sport.tennis,
    name: 'Court',
    description: 'Description',
    maxPeople: 0,
    status: CourtStatus.blocked,
    createdAt: DateTime.now(),
    updatedAt: DateTime.now(),
  );

  ProviderState get state => _state;

  Failure? get failure => _failure;

  CourtModel get court => _court;

  set state(ProviderState value) {
    _state = value;
    notifyListeners();
  }

  Future<void> getCourt(int complexId, int courtId) async {
    final bool isInitialLoad = _court.id == -1 || _state == ProviderState.error;

    if (isInitialLoad) {
      state = ProviderState.loading;
    }

    final result = await _courtsUseCases.getCourt(complexId, courtId);
    result.fold(
      (failure) {
        _failure = failure;
        // Only set to error state if it was an initial load or retrying from an error state.
        // Otherwise, keep the existing (stale) data visible.
        if (isInitialLoad) {
          state = ProviderState.error;
        }
      },
      (value) {
        _court = value;
        // Clear any previous failure on successful fetch
        _failure = null;
        state = _court.id != -1 ? ProviderState.loaded : ProviderState.empty;
      },
    );
  }
}
